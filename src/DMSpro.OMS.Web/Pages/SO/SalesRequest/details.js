$(async function () {
    ...
    var SOService = window.dMSpro.oMS.mdmService.controllers.salesOrders.salesOrder;
    SOService.getSOInfo(companyId, new Date(), null)
        .done(async result => {
    ...
    var loadControl = function (data) {
                            ...
                            editorOptions: {
                                ...
                                onValueChanged: function (e) {
                                    ...
                                    if (e.value != null) {
                                        ...
                                        var routesOfCus = data.customersRoutesDictionary[e.value];
                                        if (routesOfCus != null) {
                                            if (routesOfCus.length == 1) {
                                                var itemGroupOfCusRoute = data.customersRoutesItemGroupsDictionary[e.value][routesOfCus[0]];
                                                var itemsInItemGroup = data.itemsInItemGroupsDictionary[itemGroupOfCusRoute[0]];
                                                itemsInItemGroup.forEach(x => {
                                                    listItemOfCus.push(data.item[x]);
                                                })
                                            }
                                            else {
                                                listItemOfCus = data.itemList;
                                            }
                                        } else {
                                            listItemOfCus = data.itemList;
                                        }
                                    } else {
                                        $('.openItemsPopupButton').data('dxButton').option('disabled', true);
                                        var itemEmty = SalesRequestDetailsModel.find(x => !x.itemId);
                                        var index = SalesRequestDetailsModel.indexOf(itemEmty);
                                        SalesRequestDetailsModel.splice(index, 1);
                                        dgSalesRequestDetails.refresh();
                                    }
                                }
                            },
            ...
            jQuery.extend(dxDataGridConfiguration, {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        dataField: 'uomId',
                        calculateDisplayValue: "uomDisplay",
                        lookup: {
                            dataSource(options) {
                    ...
                    },
                    {
                        ...
                        dataField: 'vatId',
                        calculateDisplayValue: "vatDisplay",
                        lookup: {
                            dataSource() {
                                return {
                                    store: vatList,
                    ...
                    },
                    {
                        ...
                        calculateDisplayValue: 'warehouseDisplay',
                        width: 200,
                        visible: false
                    },
                    {
                        ...
                        calculateDisplayValue: 'warehouseLocationDisplay',
                        width: 200,
                        visible: false
                    }
        ...
        $("#SaveButton").click(function (e) {
            ...
            var header = SalesRequestHeaderModel;
        ...
        $('#CancelButton').click(function (e) {
            salesRequestService.cancelDoc(SalesRequestHeaderId, { contentType: "application/json" })
                .done(result => {
                    abp.message.success(l('Congratulations'));
                })
                .fail(result => {
    ...
    function addListToData(data) {
        ...
        // get customer routes
        data.customerRoutes = Object.keys(data.customersRoutesDictionary).map(function (key) {
            return data.customersRoutesDictionary[key];
        });
        // get list item group of customer routes
        data.itemGroupCustomerRoutes = Object.keys(data.customersRoutesItemGroupsDictionary).map(function (key) {
            return data.customersRoutesItemGroupsDictionary[key];
        });
    };

    function ReloadHeader() {
        var bc = new BroadcastChannel('SalesRequestHeader');
