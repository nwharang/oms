var ARCreditMemoHeaderId = sessionStorage.getItem('ARCreditMemoHeaderId');
var ARCreditMemoDetailsModel = [];
...
var loadControl = function (data) {
    ...
    const frmArCreditMemoHeaders = $('#frmArCreditMemoHeaders').dxForm({
        labelMode: "floating",
        colCount: 4,
        items: [
            {
    ...
    const dgArCreditMemoDetails = $('#dgArCreditMemoDetails').dxDataGrid(
        jQuery.extend(dxDataGridConfiguration, {
            dataSource: ARCreditMemoDetailsModel,
            ...
            onSaved: function (e) {
                calculatorDocTotal();
                //var sumDiscountAmt = dgArCreditMemoDetails.getTotalSummaryValue('discountAmt');
                //var sumLineAmt = dgArCreditMemoDetails.getTotalSummaryValue('lineAmt');
                //var sumLineAmtAfterTax = dgArCreditMemoDetails.getTotalSummaryValue('lineAmtAfterTax');
                //frmArCreditMemoHeaders.updateData('docTotalLineDiscountAmt', sumDiscountAmt);
                //frmArCreditMemoHeaders.updateData('docTotalLineAmt', sumLineAmt);
                //frmArCreditMemoHeaders.updateData('docTotalLineAmtAfterTax', sumLineAmtAfterTax);
            ...
            onContentReady: function (e) {
                ...
                //if ($('#frmArCreditMemoHeaders').data('dxForm').getEditor('businessPartnerId').option('value') != null) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    setCellValue: function (newData, value, currentData) {
                        var formSalesRequest = $('#frmArCreditMemoHeaders').data('dxForm');
    ...
    $("#SaveButton").click(function (e) {
        ...
        var header = frmArCreditMemoHeaders.option('formData');
        header.companyId = companyId;
        header.linkedSFAId = linkedSFAId;

        //var salesRequestDetails = dgArCreditMemoDetails.getDataSource().items();
        var salesRequestObject = {
            header: header,
            details: removeEmtyDetail(JSON.parse(JSON.stringify(ARCreditMemoDetailsModel)))
        };

        console.log("Save data: ", salesRequestObject);

        if (ARCreditMemoHeaderId) {
            arCreditMemoService.updateDoc(ARCreditMemoHeaderId, salesRequestObject, { contentType: "application/json" })
                .done(result => {
            ...
            arCreditMemoService.createDoc(salesRequestObject, { contentType: "application/json" })
                .done(result => {
                    abp.message.success(l('Congratulations'));
                    ARCreditMemoHeaderId = result.header.id;
                    sessionStorage.setItem('ARCreditMemoHeaderId', ARCreditMemoHeaderId);
                    console.log(result);
                    ReloadHeader();
                })
                .fail(result => {
...
function calculatorDocTotal() {
    ...
    var dataRows = ARCreditMemoDetailsModel;
...
function appendSelectedItems(selectedItems) {
    selectedItems.forEach(u => {
        ...
        ARCreditMemoDetailsModel.unshift({
            id: u.id,
            itemId: u.id,
            vatId: u.vatId,
    ...
    $('#dgArCreditMemoDetails').data('dxDataGrid').refresh();
    calculatorDocTotal();
}
